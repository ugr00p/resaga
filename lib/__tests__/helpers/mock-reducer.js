...
const initialState = {};
const nestedValueState = initialState.merge(
  initialState.concat({
    [key]: {
      deep: {
        nested: 123,
...
const submitFailed = submitFormFailed(
  error,
  payload,
  name,
  requestName,
...
const submitSuccess = submitFormSucceed(
  result,
  payload,
  name,
  requestName,
...
const setFunctionNested = setVariableWithFunction(
  [false, key, 'deep', 'nested'],
  func,
);
...
const expectSetFunctionNormalNested = state =>
  state.merge(
    state.concat({
      [key]: {
        deep: {
