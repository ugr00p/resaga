...
export function beforeSubmitForm(
  pageName,
  requestName,
  payload,
  options = { configs: {} },
  ...
  return {
    type: `${helpers.padEnd(
      BEFORE_DISPATCH,
    )}${SEPARATOR}${name}${SEPARATOR_SHORT}${requestName}${isRemote}`,
    name,
    requestName,
    payload,
    options,
...
export function submitForm({
  ...
  return {
    type: `${helpers.padEnd(
      DO_SUBMIT,
    )}${SEPARATOR}${page}${SEPARATOR_SHORT}${requestName}`,
    name: page,
    requestName,
    payload,
    options,
...
export function submitFormFailed(error, payload, name, requestName, options) {
  return {
    type: `${helpers.padEnd(
      SUBMIT_FAILED,
    )}${SEPARATOR}${name}${SEPARATOR_SHORT}${requestName}${SEPARATOR_SHORT}failed`,
    name,
    requestName,
    payload,
    options,
...
export function submitFormSucceed(result, payload, name, requestName, options) {
  return {
    type: `${helpers.padEnd(
      SUBMIT_SUCCEED,
    )}${SEPARATOR}${name}${SEPARATOR_SHORT}${requestName}${SEPARATOR_SHORT}successfully`,
    name,
    requestName,
    payload,
    options,
...
export function doAcknowledge(name, requestName) {
  return {
    type: `${helpers.padEnd(
      SUBMIT_ACKED,
    )}${SEPARATOR}${name}${SEPARATOR_SHORT}${requestName}`,
    name,
    requestName,
  };
}
...
export function setVariable(keyPath, value) {
  return {
    type: `${helpers.padEnd(REDUX_SET)}${SEPARATOR}${
      keyPath[0]
    }${SEPARATOR_SHORT}${keyPath[1]}`,
    keyPath,
    value,
    name: keyPath[0],
  };
...
export function setVariableWithFunction(keyPath, func) {
  return {
    type: `${helpers.padEnd(REDUX_SET_FN)}${SEPARATOR}${
      keyPath[0]
    }${SEPARATOR_SHORT}${keyPath[1]}`,
    keyPath,
    func,
