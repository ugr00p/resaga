...
describe('setValue()', () => {
  const props = {
    internalProps: {
      setValue: jest.fn((page, key, value, cb) => {
        if (typeof cb === 'function' && key !== 'donotset')
          cb({ [key]: value });
      }),
  ...
  it('should set value', () => {
    expect(setValue(props, key, value)).toBe(
      props.internalProps.setValue(key, value),
    );
  });
  ...
  it('should set by setValue config', () => {
    ...
    expect(setValue(setValueProps, key, value)).toBe(
      setValueProps.internalProps.setValue(key, value),
    );
  });
  ...
  it('should set object with function', () => {
    expect(setValue(props, { [key]: functionValue })).toBe(
      props.internalProps.setValueWithFunc(key, functionValue),
    );
  });
