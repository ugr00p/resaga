...
describe('analyseNextProps', () => {
  ...
  describe('analyseRequest()', () => {
    ...
    it('request is object, prop error', () => {
      ...
      expect(res).toBe(true);
      expect(requestObject.onError).toBeCalledWith(
        propsError[SERVER_ERROR],
        payload,
      );
    ...
    it('request is object, prop success', () => {
      ...
      expect(requestObject.onSuccess).toBeCalledWith(
        propsSuccess[RESULT],
        payload,
      );
    ...
    it('request is object, callback defined', () => {
      const callback = jest.fn();
      const res = analyseRequest(
        requestObject,
        { options: { callback }, ...propsSuccess },
        options,
      );
      expect(res).toBe(true);
      expect(callback).toBeCalled();
  ...
  describe('analyseNextProps()', () => {
    ...
    const result = 'result';
    const payload = 'payload';
    const store = {}.set(requestName, {
      [SUBMIT_SUCCESS]: true,
      [RESULT]: result,
      [PAYLOAD]: payload,
    });
    const storeIsLoading = {}.set(requestName, { isLoading: true });
    ...
    it('no actions', () => {
      analyseNextProps({ someStore: store }, undefined, {
        acknowledge,
        configs,
      });
      expect(acknowledge).not.toBeCalled();
    ...
    it('store found, isLoading true', () => {
      analyseNextProps({ [name]: storeIsLoading }, actions, {
        acknowledge,
        configs,
      });
      expect(acknowledge).not.toBeCalled();
    ...
    it('ack true', () => {
      analyseNextProps({ [name]: store }, actionsSuccess, {
        acknowledge,
        configs,
      });
