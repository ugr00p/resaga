...
const simplifyResaga = configs => Component => {
  class SimplifyResaga extends PureComponent {
    componentWillMount = () => {
      const { setValue, setValueWithFunc, beforeDispatch } = this.props;
    ...
    componentWillReceiveProps = nextProps => {
      if (debug.on()) {
        const name = `${this.internalProps.Component.name}${
    ...
    componentWillUpdate = nextProps => {
      const changes = helpers.getValuesChanged(
        this.values,
        nextProps,
        this.internalProps,
      );
  ...
  const mapDispatch = dispatch => ({
    ...
    setValueWithFunc: (...params) =>
      dispatch(actions[SET_VARIABLE_FN](...params)),
    beforeDispatch: (...params) => dispatch(beforeSubmitForm(...params)),
  });

  SimplifyResaga.displayName = `WithSimplifyResaga(${utils.getDisplayName(
    Component,
  )})`;
