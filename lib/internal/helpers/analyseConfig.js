...
const setConfig = (page, cfg) => {
  if (resagaConfigs[page])
    errors.breakingError(
      `Component '${page}' has been mounted multiple times which is anti-pattern. It's possibly due to dynamically generating by a loop. Consider moving state up to parent.`,
    );
...
const checkConfig = (configs, name, requestName) => {
  if (!configs)
    throw SyntaxError(
      `Component '${name}' not found. Check whether the component has been unmounted before the request finished, or it has never been mounted in the first place.`,
    );
  if (!configs.requests)
    throw SyntaxError('`configs.requests` must be defined.');
  if (requestName && !configs.requests[requestName])
    throw SyntaxError(
      `Request '${requestName}' must be defined in configs.requests.`,
    );
...
const getConfig = (name, requestName) => {
  if (!name)
    return errors.error(
      `The configuration of '${name}' not found. Check whether some component has been unmounted before the request finished, or it has never been mounted in the first place.`,
    );
...
const analyse = (
  ...
  if (typeof postSuccessEach === 'object')
    postSuccessEach = postSuccessEach[requestName];
  if (typeof postErrorEach === 'object')
    postErrorEach = postErrorEach[requestName];
...
const analyseConfigs = originConfigs => {
  ...
  if (configs.page && !configs.name) {
    ...
    errors.warning(
      '`CONFIG.page` is renamed to `CONFIG.name`. Please adapt your code. `CONFIG.page` will not be supported in the next minor release.',
    );
  ...
  if (configs.submit && !configs.requests) {
    // TODO: backward compatibility v0.0.x
    errors.warning(
      '`CONFIG.submit` is renamed to `CONFIG.requests`. Please adapt your code. `CONFIG.submit` will not be supported in the next minor release.',
    );
