...
const isGetter = ({ keyPath, getter }) =>
  SELECTOR_UTILS.isInvalidKeyPath(keyPath) && typeof getter === 'function';

const isCacheKey = cacheKey =>
  typeof cacheKey === 'function' || Boolean(cacheKey);

const isInvalidCacheKey = keyPath => !SELECTOR_UTILS.isCacheKey(keyPath);

// TODO: check all elements
const isPropSelector = props =>
  (Array.isArray(props) && isPropSelector(first(props))) ||
  typeof props === 'function';

// TODO: check all elements
const isArrayOfKeyPaths = keyPath =>
  Array.isArray(keyPath) && SELECTOR_UTILS.isKeyPath(first(keyPath));
...
const isInvalid = selector => {
  ...
  return (
    SELECTOR_UTILS.isInvalidKeyPath(selector.keyPath) &&
    SELECTOR_UTILS.isInvalidCacheKey(selector.cacheKey)
  );
};
...
const selectKeyPath = ({ keyPath, notSetValue }, isLoading) => (
  ...
  if (helpers.isResagaStore(name) && !isLoading) {
    reducerHelper.keyPathToStringPath(key);
    value = dotProp.get(state, `${name}.${VARIABLES}.${key}`, notSetValue);
  } else {
    value = dotProp.get(state, `${name}.${key}`, notSetValue);
...
const makeStateSelectors = (notSetValue, isLoading) => (accumulate, keyPath) =>
  accumulate.concat(
...
const normaliseKeyPath = ({ keyPath, notSetValue } = {}, props, isLoading) => {
  ...
  if (SELECTOR_UTILS.isArrayOfKeyPaths(keyPath)) {
    ...
    return keyPath.reduce(
      SELECTOR_UTILS.makeStateSelectors(notSetValue, isLoading),
      [],
    );
  }
  ...
  if (typeof keyPath === 'function') {
    return normaliseKeyPath(
      { keyPath: keyPath(props), notSetValue },
      props,
      isLoading,
    );
  }
...
const customSelectProp = func => (_, props) =>
  typeof func === 'function' ? func(props) : undefined;
...
const normaliseCacheKey = cacheKey =>
  cacheKey
    ? SELECTOR_UTILS.customCacheKey(cacheKey)
    : SELECTOR_UTILS.defaultCacheKey;

// no cache for un-optimised selectors
const normaliseOptions = ({ cacheKey, options = {} }) =>
  cacheKey ? { cacheObject, ...options } : options;
...
const select = ({ keyPath, props, cacheKey, options, getter }) => {
  let inputSelectors = [];
