  ...
  if (originConfigs.name === name) {
    ...
    try {
      ...
      callbacks = makeCallback(
        submitFormSucceed,
        configs.onSuccess, // actions to call on success
        configs.postProcess(result, payload),
        payload,
        configs.name,
        requestName,
        options, // parameters
      ...
      callbacks = makeCallback(
        submitFormFailed,
        configs.onError, // actions to call on error
        configs.postProcessError(error, payload),
        payload,
        configs.name,
        requestName,
        options, // parameters
